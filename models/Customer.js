...
customerSchema.pre('save', async function(next) {
  if (this.isNew && !this.id) {
    try {
      ...
      if (lastCustomer && lastCustomer.id && !isNaN(lastCustomer.id) && typeof lastCustomer.id === 'number') {
        nextId = lastCustomer.id + 1;
      } else if (lastCustomer) {
        // If lastCustomer exists but has invalid ID, find the highest valid ID
        const validCustomers = await this.constructor.find({ 
          id: { $type: 'number', $ne: null } 
        }).sort({ id: -1 }).limit(1);
        
        if (validCustomers.length > 0 && !isNaN(validCustomers[0].id)) {
          nextId = validCustomers[0].id + 1;
        }
      }
      
      this.id = nextId;
      console.log(`🆔 Assigned new customer ID: ${this.id}`);
    } catch (error) {
